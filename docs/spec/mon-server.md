...
| `mon_clients` | `id` (slug ≤ 32 `[A-Za-z0-9_-]`) PK | `name`, `region`, `paths` (JSON словаря §5.1, default `["direct","hops"]`), `token_hash` (SHA-256), `enabled`, `state` (`ONLINE`/`OFFLINE`/`NEVER`), `last_heartbeat`, `version`, `xray_version`, `applied_revision`, `config_error`, `config_error_at`, `rejected_targets` (JSON `[{target, error}]` из последнего heartbeat), `remote_ip`, `approved_at`, `missed_heartbeats` | реестр |
...
- Default при одобрении (admin UI §9.2 и авто-одобрение из ansible) — `["direct","hops"]`. Явные имена ограничивают коробку частью звеньев (inner доступен не из каждого региона); `direct` на коробках во враждебных регионах владелец снимает, чтобы не светить настоящий адрес real server как Reality-endpoint.
...
**Звено ушло из пробируемых** — удалено, переименовано (переименование = удаление + добавление), перешло в `pending` или `draining` — или с появлением цепочки ушёл `proxy`: строки `targets` этих path'ов mon-server удаляет молча, без событий панели и без Telegram; свои строки звена панель чистит каскадом сама ([proxy-chain.md](https://github.com/SBKubric/3ax-ui-proxy/blob/main/docs/spec/proxy-chain.md) §6.1). Это не `path_removed`: та пауза — для path'а, который панель по-прежнему пробирует, но который убран из `paths` mon-client (§7.2).

**AWG probe-пиры** — на mon-client × path, в том числе на каждое звено (правило [#80](https://github.com/SBKubric/3ax-ui-monitoring/issues/80)). Сколько пиров выдать и кому, решает панель: сверх её настройки `monProbePeerLimit` пиры не выдаются (порядок выдачи — контракт §4.3), и mon-client × path без пира получает `PAUSED no_probe_link` (§7.2).
